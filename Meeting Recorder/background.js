...
(function() {
    ...
    chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
        if (changeInfo.status === "complete" && tab.url) {
            ...
            if (service === 'gmeet') {
                ...
                chrome.storage.local.get(['isExtendingToMeet', 'extendTransitionTime'], async (result) => {
                    if (result.isExtendingToMeet) {
                        ...
                        if (timeSinceExtend < 10000) {
                            ...
                            setTimeout(() => {
                                startRecordingForTab(tabId, 'gmeet', true, true, true);
                            ...
                            chrome.tabs.sendMessage(tabId, { 
                                ...
                                message: "🎬 Recording continued from Huddle",
                                duration: 5000
                            });
    ...
    // ==================== COMMON FUNCTIONS ====================
   function startRecordingForTab(tabId, service, isAuto = false, forceRecord = false, extendedFromHuddle = false) {
        if (currentRecordingTab && !isAuto && !forceRecord) {
            console.log("⚠️ Already recording in tab:", currentRecordingTab);
            return;
        }

        console.log(`🎬 Starting recording for ${service} tab:`, tabId, `(auto: ${isAuto}, force: ${forceRecord})`);
