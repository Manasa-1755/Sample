...
(function() {
   ...
   async function startRecording(tabId) {
        console.log(`🎬 Starting recording for ${currentService} tab:`, tabId, `(force: ${isForceRecord})`);
        
        await syncToggleState();

        // 🔥 ONLY block if this is NOT a forced recording
        if (!isForceRecord) {
            const blockResult = await chrome.storage.local.get(['blockMeetAutoRecord', 'blockMeetAutoRecordUntil']);
            ...
            if (isBlocked) {
                console.log("🔒 Recorder start BLOCKED - Extending from Huddle");
                return;
            }
        } else {
            console.log("🔥🔥🔥 FORCED RECORDING - IGNORING BLOCK! 🔥🔥🔥");
        }
    ...
    const handleAsync = async () => {
        try {
            if (message.action === "startRecording") {
                ...
                isForceRecord = message.forceRecord || false; 
                isExtendedFromHuddle = message.extendedFromHuddle || false;
                currentTabId = message.tabId;
                currentService = message.service || 'gmeet';
                console.log("🎬 Starting recording, service:", currentService, "auto mode:", isAutoRecord, "force mode:", isForceRecord, "tabId:", currentTabId);
                setupServiceBadge();
